...
```powershell
.\scripts\release-full.ps1 -Version "1.3.0" -ReleaseNotes "Mo ta thay doi"
```
...
.\create-release.ps1 -Version "1.3.0"
```
...
- Hiển thị dưới cùng mọi tab (kéo giãn chiều cao, bo góc card)
...
- Tab **Handbook** trên rail: kiến trúc source Loyalty (folder, layer ABP, port, luồng gateway)

### Kubernetes

- Tab **Kubernetes** trên rail: dashboard nhúng (WebView2), đọc kubeconfig trên máy
- Chọn cluster → xem Pods, Deployments, Services, … (giống Lens)
- **Terminal nhúng**: nhiều tab PowerShell, kéo resize chiều cao, `az login` / `get-credentials` trong tool
- **Đăng nhập cloud** mở terminal; **Quét lại** sau khi cập nhật kubeconfig
- Chuyển tab Services ↔ Kubernetes **không ngắt** kết nối cluster
- Cấu hình cluster: `k8s.local.json` (local, gitignored) — mẫu `k8s.clusters.example.json`
- Shell pod: mở Windows Terminal (cần TTY tương tác)
...
| `k8s.local.json` | Cluster K8s đã cấu hình (local, gitignored) |
...
| `backup-*.json`, `defaults/local.*.json`, `paths.local.json`, `k8s.local.json` | **Không commit / không share** — backup giữ full config cho restore máy local |
